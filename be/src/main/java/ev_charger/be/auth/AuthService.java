...
public class AuthService {
    ...
    public SocialLoginResponse socialLogin(String accessToken, Provider provider) {
        ...
        Optional<User>existing = userRepository.findByProviderAndProviderId(provider, KAKAO, kakaoUer.id()) //우리 DB에 이 사람이 이미 있는지 확인 provider랑 카카오에서 받은 id 조합으로 찾음
        if (existing.isPresent()){ // DB에 있으면 기존유저 없으면 신규유저
            ...
            return SocialLoginRespone.builder() //sucess랑 토큰 2개 반환
                    .status("success")
